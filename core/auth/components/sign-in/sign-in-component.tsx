...
const SignInComponent: FunctionComponent<{isOpened: boolean, handleClose: (data?: any) => void}> = function({isOpened, handleClose}) {
  const { register, control, handleSubmit, watch, errors } = useForm<LoginFormType>()

  const onSignIn = (data: LoginFormType) => {
  ...
  return (
    <Dialog open={isOpened} onClose={handleClose} aria-labelledby="form-dialog-title">
      ...
      <DialogActions>
        ...
        <Button onClick={handleSubmit(onSignIn)} color="primary">
          Subscribe
        </Button>
      </DialogActions>
    </Dialog>
